...
export default {
  ...
  data() {
    return {
      ...
      validatorRules: {
        taskid: { rules: [{ required: true, message: '请输入任务!' }] },
        chiefid: { rules: [{ required: true, message: '请输入分管领导!' }] },
        summaryTime: {
          rules: [
        ...
        content: { rules: [{ required: true, message: '请输入任务小结!' }] },
        contentAnnex: { rules: [] }
      },
      url: {
  ...
  methods: {
    ...
    isDateValid() {
      if (this.summaryTime <= this.getNow()) return true
