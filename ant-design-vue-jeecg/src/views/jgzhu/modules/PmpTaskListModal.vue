    ...
    <a-spin :spinning="confirmLoading">
      <a-form :form="form">
        ...
        <a-form-item label="任务名称" :labelCol="labelCol" :wrapperCol="wrapperCol">
          <a-input v-decorator="[ 'projectname', validatorRules.projectname]" placeholder="请输入任务名称" :trigger-change="true"></a-input>
        </a-form-item>
  ...
  export default {
    ...
    data () {
      return {
        ...
        validatorRules:{
          ...
          projectname:{rules: [{ required: true, message: '请输入任务名称!' },
            { validator: (rule, value, callback) => validateDuplicateValue('pmp_project', 'projectname', value, this.model.id, callback)}]}
        },
        url: {
    ...
    methods: {
      ...
      edit (record) {
        ...
        this.visible = true;
        this.$nextTick(() => {
          this.form.setFieldsValue(pick(this.model,'parentnode','projectname'))
        })
      },
      close () {
        this.$emit('close');
      ...
      popupCallback(row){
        this.form.setFieldsValue(pick(row,'parentnode','projectname'))
      },
