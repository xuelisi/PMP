...
public class FindsDepartsChildrenUtil {
     ...
     */
    public static List<PmpProjectTreeModel> myWrapTreeDataToTreeList(List<PmpProject> recordList) {
        // 在该方法每请求一次,都要对全局list集合进行一次清理
        //idList.clear();
        List<DepartIdModel> idList = new ArrayList<DepartIdModel>();
        List<PmpProjectTreeModel> records = new ArrayList<>();
        for (int i = 0; i < recordList.size(); i++) {
            PmpProject depart = recordList.get(i);
            records.add(new PmpProjectTreeModel(depart));
        }
        List<PmpProjectTreeModel> tree = myFindChildren(records, idList);
        mySetEmptyChildrenAsNull(tree);
        return tree;
    }
     ...
     */
    private static List<PmpProjectTreeModel> myFindChildren(List<PmpProjectTreeModel> recordList,
                                                         List<DepartIdModel> departIdList) {

        List<PmpProjectTreeModel> treeList = new ArrayList<>();
        for (int i = 0; i < recordList.size(); i++) {
            PmpProjectTreeModel branch = recordList.get(i);
            if (branch.getParentnode().equals("0")) {
                treeList.add(branch);
                DepartIdModel departIdModel = new DepartIdModel().convert1(branch);
                departIdList.add(departIdModel);
            }
        }
        myGetGrandChildren(treeList,recordList,departIdList);
     ...
     */
    private static void myGetGrandChildren(List<PmpProjectTreeModel> treeList,List<PmpProjectTreeModel> recordList,List<DepartIdModel> idList) {

        for (int i = 0; i < treeList.size(); i++) {
            PmpProjectTreeModel model = treeList.get(i);
            DepartIdModel idModel = idList.get(i);
            for (int i1 = 0; i1 < recordList.size(); i1++) {
                PmpProjectTreeModel m = recordList.get(i1);
                if (m.getParentnode()!="0" && m.getParentnode().equals(model.getId())) {
                    model.getChildren().add(m);
                    DepartIdModel dim = new DepartIdModel().convert1(m);
                    idModel.getChildren().add(dim);
                }
            }
            myGetGrandChildren(treeList.get(i).getChildren(), recordList, idList.get(i).getChildren());
        }
     ...
     */
    private static void mySetEmptyChildrenAsNull(List<PmpProjectTreeModel> treeList) {
        ...
        for (int i = 0; i < treeList.size(); i++) {
            ...
            if (model.getChildren().size() == 0) {
                ...
                mySetEmptyChildrenAsNull(model.getChildren());
